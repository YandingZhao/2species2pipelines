suppressPackageStartupMessages({
  library(Seurat)
  library(harmony)  # v2 from GitHub (immunogenomics/harmony)
})
...
if (normalization == "sctransform") {
  ...
  merged <- do.call(RunHarmony, c(list(object = merged), H2_ARGS))
...
pca_out    <- paste0(sample_id, "_harmony_v2_embedding.tsv")
rds_out    <- paste0(sample_id, "_harmony_v2_integration.rds")
report_out <- paste0(sample_id, "_harmony_v2_report.txt")
...
writeLines(
  c(
    ...
    paste("harmony_version:", as.character(packageVersion("harmony"))),
