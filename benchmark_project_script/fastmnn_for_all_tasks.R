### this script is to run the fastmnn for the integration
### the usage is to run the script in the command line with the following command
### Rscript fastmnn_for_all_tasks.R 'task3_' 

### the script will read the data from the folder ../data/process_data/rds_file/ which store the RDS file for the data
### the output data would be store in the folder ../output/method_outputs/fastmnn/
...
setwd('./script')
source('./core_script/fastMNN_process.R')
...
for (target in targets){
    ...
    if (target %in% c('task4_')){
        ...
        common_genes=intersect(rownames(input_b_rds),rownames(input_a_rds))   
        ...
        input_a_rds@assays$RNA@meta.features <- data.frame(row.names = rownames(input_a_rds[["RNA"]]))
        input_b_rds@assays$RNA@meta.features <- data.frame(row.names = rownames(input_b_rds[["RNA"]]))
    } else if (target %in% c('task17_','task17-2_')){
        ...
        mapping=read.csv('../benchmark/OrthoFinder/one2one_orthologs/cat_to_dog.csv',header=T)
        mapping=mapping[!duplicated(mapping[,c(4,5)]),]  
        a_overlap=intersect(rownames(input_a_rds),mapping$cat_gene)
        input_a_rds=input_a_rds[mapping$cat_gene,]  
    ...
    target_list=merge(input_a_rds,input_b_rds)
    print('running started!')
    integrated_list=mnn_preprocess(target_list,task_name=gsub("_","",target),batch_label=batch_label,celltype_label=celltype_label,
                                    save_prefix='../output/method_outputs/fastmnn/')
