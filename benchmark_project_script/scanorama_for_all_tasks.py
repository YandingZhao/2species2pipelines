## in this code, we will use scanorama to integrate the datasets from different species
## to use the code, we can run the command like below:
## python scanorama_for_all_tasks.py --all_targets task3_ task4_ 
...
save_dirname = '../output/method_outputs/scanorama/'
print(save_dirname)
...
def main():
    parser = argparse.ArgumentParser(description='scanorama for all tasks')
    ...
    for target in all_targets:
        ...
        # load the one2one orthologous mapping table
        ...
        for adata in [adata_a, adata_b]:
            ...
            sc.pp.log1p(adata)
            sc.pp.scale(adata)
        adata_ls = [adata_a, adata_b]
        corrected = scanorama.correct_scanpy(adata_ls,batch_size=30,return_dense=True, knn=10,return_dimred=True,seed=seed)
        ...
        total_ann.obs['celltype'] = corrected[0].obs['celltype'].tolist() + corrected[1].obs['celltype'].tolist()
        total_ann.obs['batch'] = corrected[0].obs['batch'].tolist() + corrected[1].obs['batch'].tolist()
   
        sc.tl.pca(total_ann, svd_solver='arpack', n_comps=30)
        total_ann.obsm['X_pca'] *= -1  # multiply by -1 to match Seurat
        total_ann.obs['batch']=total_ann.obs['batch'].astype('str')
        
        end_time=time.time()
        elapsed_time = end_time - start_time
        with open(f"../output/method_outputs/scanorama/scanorama_timing_log.txt", "a") as file:
            file.write(f"{re.sub('_', '', target)}, seed: {seed}, Elapsed time: {elapsed_time:.2f} seconds\n")

        total_ann.write_h5ad(os.path.join(save_dirname,target+str(seed)+'.h5ad'))

        # save the pca files  
        filename = target+str(seed)+'_scanorama_pca.txt'
        coln_pca = []
        for i in range(total_ann.obsm['X_pca'].shape[1]):
            coln_pca.append("X_pca"+str(i+1))
        coln_pca.append('batchlb')
        coln_pca.append('celltype')
        target_obj=pd.DataFrame(total_ann.obsm['X_pca'])
        target_obj['batch']=total_ann.obs['batch'].values
        target_obj['celltype']=total_ann.obs['celltype'].values
        target_obj.columns=coln_pca
        total_ann.obs_names_make_unique()
        target_obj.index=total_ann.obs_names
