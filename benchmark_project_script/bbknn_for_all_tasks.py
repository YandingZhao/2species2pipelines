# in this script, we run bbknn for the integration
# usage of the script is like:
# python bbknn_for_all_tasks.py --all_targets <task_id> 
# eg. python bbknn_for_all_tasks.py --all_targets task4_ task6_
# we will read the h5ad files from the ../data/process_data/h5ad_files folder
import numpy as np
import bbknn
import pandas as pd
...
save_dirname = '../output/method_outputs/bbknn/'
print(save_dirname)
...
def main():
    parser = argparse.ArgumentParser(description='bbknn for all tasks')
    ...
    for target in all_targets:
        ...
        adata_a.obs[batch_label]=adata_a.obs[batch_label].astype(str)
        adata_b.obs[batch_label]=adata_b.obs[batch_label].astype(str)
        ...
        # read the one2one orthologous genes mapping table
        ...
        sc.pp.scale(adata_ls)
        sc.tl.pca(adata_ls, svd_solver='arpack', n_comps=30)
        adata_bbknn=bbknn.bbknn(adata_ls,batch_key='batch',copy=True,neighbors_within_batch=5,
        ...
        adata_bbknn.obs['orig.ident']=adata_bbknn.obs['batch']

        end_time=time.time()
        elapsed_time = end_time - start_time
        with open(f"../output/method_outputs/bbknn/bbknn_timing_log.txt", "a") as file:
            file.write(f"{re.sub('_', '', target)}, seed: {seed}, Elapsed time: {elapsed_time:.2f} seconds\n")

        adata_bbknn.write_h5ad(os.path.join(save_dirname,target+str(seed)+'.h5ad'))        
main()