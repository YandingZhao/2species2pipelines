...
## in this code, we will use scVI to perform the integration of the datasets
## usage of the code is like below:
## python scVI_for_all_tasks.py --all_targets task3_ 
## the output (embeddings) is saved in the output/methods_output/scVI folder

sc.settings.verbosity = 3  # verbosity: errors (0), warnings (1), info (2), hints (3)
sc.logging.print_versions()

# Create folder to save the results
save_dirname = '../output/methods_output/scVI/'
print(save_dirname)
...
def main():
    parser = argparse.ArgumentParser(description='scVI for all tasks')
    ...
    for target in all_targets:
        ...
        adata_a.obs['batch']=adata_a.obs[batch_label]
        adata_a.obs['celltype']=adata_a.obs[celltype_label]
        adata_b.obs['batch']=adata_b.obs[batch_label]
        adata_b.obs['celltype']=adata_b.obs[celltype_label]
        ...
        # load the one2one orthologous gene mapping table
        ...
        sc.pp.normalize_total(adata_ls, target_sum=1e4)
        sc.pp.log1p(adata_ls)
        adata_ls.raw = adata_ls
        adata_ls.obs['batch'] = adata_ls.obs['batch'].astype(str)
        sc.pp.highly_variable_genes(adata_ls,
                                    n_top_genes=2000,
                                    subset=True,
                                    layer="counts",
                                    flavor="seurat_v3",
                                    batch_key="batch")
        scvi.model.SCVI.setup_anndata(adata_ls,
                                        layer="counts",
                                        batch_key='batch')
        model = scvi.model.SCVI(adata_ls,n_latent=30)
        model.train()
        # save model
        model.save(os.path.join(save_dirname,re.sub('_','',target)+"_seed_"+str(seed)+'.pt'))
        # load model
        # model = scvi.model.SCVI.load(path,adata=adata,use_gpu=True)
        latent = model.get_latent_representation()
        adata_ls.obsm['X_scVI']=latent
        adata_ls.obs['orig.ident']=adata_ls.obs['batch']
        end_time=time.time()
        elapsed_time = end_time - start_time
        with open(f"../output/method_outputs/scVI/scVI_timing_log.txt", "a") as file:
            file.write(f"{re.sub('_', '', target)}, seed: {seed}, Elapsed time: {elapsed_time:.2f} seconds\n")

        adata_ls.write_h5ad(os.path.join(save_dirname,target+str(seed)+'.h5ad'))

        # save the embedding files 
        filename = target+str(seed)+'_scVI_embedding.txt'
        coln_pca = []
        for i in range(adata_ls.obsm['X_scVI'].shape[1]):
            coln_pca.append("X_scVI"+str(i+1))
        coln_pca.append('batchlb')
        coln_pca.append('celltype')
        target_obj=pd.DataFrame(adata_ls.obsm['X_scVI'])
        target_obj['batch']=adata_ls.obs['batch'].values
        target_obj['celltype']=adata_ls.obs['celltype'].values
